...
Modified from: https://github.com/informatics-lab/jade-dask/blob/master/kubernetes/adaptive.py
BSD-3
"""
...
class KubeCluster(object):
    ...
    def scale_down(self, workers):
        ...
        try:
            pods = self.api.list_namespaced_pod(
                self.namespace, label_selector=self.worker_labels)
        except client.rest.ApiException:
            self.log.exception(
                'Error fetching existing pods. No scaling attempted.')
            return
        ...
        if not to_delete:
            return
        self.log.info("Deleting %s of %s pods",
                      len(to_delete), len(pods.items))
        for pod in to_delete:
            try:
                ...
                if e.status != 404:
                    self.log.exception(
                        'Error deleting pod: %s', pod.metadata.name)
            else:
