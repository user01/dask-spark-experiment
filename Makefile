...
spark-build:
	docker build -t user01e/sparktrial:v002 -f Dockerfile.spark .
	docker push user01e/sparktrial:v002

spark-master:
	docker run --rm -it -p 8080:8080 -p 7077:7077 user01e/sparktrial:v002 bin/spark-class org.apache.spark.deploy.master.Master

spark-worker:
	docker run --rm -it user01e/sparktrial:v002 bin/spark-class org.apache.spark.deploy.worker.Worker spark://$(IP):7077 -c 1 -m 1GB

spark-notebook:
	docker run --rm -it -p 9999:8888 user01e/sparktrial:v002 jupyter notebook --ip=0.0.0.0 --allow-root
